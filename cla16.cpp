...
// Vice-versa
unsigned int bitsToInt(bool *in, int bits)
{
...
int main()
{
    ...
    //unsigned long int totalLines = pow(2, INPUTS*BITS);
    unsigned long int totalLines = 32;
    ...
    unsigned long int input[totalLines][INPUTS];
    unsigned long int output[totalLines][OUTPUTS];
    ...
    for (int ii = 0; ii < (totalLines*2); ++ii)
    {
        ...
        intToBits(i * pow(10, 7) * 7, INPUTS*BITS, &inputBitArray[i][0]);
